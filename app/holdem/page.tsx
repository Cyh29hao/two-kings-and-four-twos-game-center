"use client";
import {TurnClock,TableDetailsTrigger} from '@/components/table-ui';
...
export default function Holdem(){
 ...
 {Array.from({length:g.rules.capacity},(_,relative)=>{const index=(Math.max(seat,0)+relative)%g.rules.capacity,s=g.seats[index],position=holdemSeatPosition(g.rules.capacity,relative),style={'--x':`${position.x}%`,'--y':`${position.y}%`} as CSSProperties;return <section key={s?.id??index} style={style} className={`th-seat ${g.turn===index&&g.phase==='playing'?'is-turn':''} ${s?.folded?'is-folded':''} ${index===seat?'is-me':''} ${position.y<35?'is-upper':''} ${position.x>50?'is-right':''}`} aria-label={(s?.name||'空位')+'的座位'}>{s?<><PlayerChat bubble={bubbles[s.id]} align="center"><span className="avatar">{s.name.slice(0,1)}</span></PlayerChat><b className="th-player-name" title={s.name}>{index===seat?'你':s.name}{s.bot&&<small>人机</small>}</b><div className="th-position">{g.button===index&&<em>D</em>}{g.smallSeat===index&&<span>小盲</span>}{g.bigSeat===index&&<span>大盲</span>}</div><>{index!==seat&&<strong className="th-stack" aria-label={`剩余 ${chips(s.stack)} 筹码`}>{chips(s.stack)}</strong>}{index!==seat&&<div className="th-hole" aria-label={s.name+'的底牌'}>{s.hand.length?s.hand.map(c=><HoldemCard key={c} card={c} size="seat"/>):[0,1].map(i=><span className="th-card-back" key={i}/>)}</div>}</>{g.phase==='playing'&&g.turn===index?index!==seat&&<TurnClock seconds={Math.min(g.rules.seconds,remaining)} label={s.name+'正在行动'}/>:<span className="th-seat-action">{['waiting','finished'].includes(g.phase)?s.ready?'已准备':'':s.folded?'弃牌':s.allIn?'全下':s.bet==='0'?s.last:''}</span>}{['playing','runout'].includes(g.phase)&&s.bet!=='0'&&<span className="th-bet">{chips(s.bet)}</span>}{s.bot&&!g.fixed&&g.host===user.id&&<button className="th-remove" disabled={busy} onClick={()=>act('remove_bot',{botId:s.id})}>移除人机</button>}</>:<div className="th-empty"><Users size={26}/><span>等一位朋友</span>{g.host===user.id&&!g.fixed&&<Button size="sm" variant="outline" disabled={busy} onClick={()=>act('add_bot')}><Bot size={14}/>添加人机</Button>}</div>}</section>})}</section></div>
 <HoldemControls game={g} userId={user.id} roomCode={room.code} connected={connection.code===room.code&&connection.ready} reset={controlReset} seconds={remaining} busy={busy} onAction={act}><RoomChat compact key={room.code} userId={user.id} closed={g.phase==='closed'} code={room.code} onSnapshot={onSnapshot}/></HoldemControls>
