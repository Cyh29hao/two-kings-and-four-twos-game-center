...
function ResultDetails({result:r}:{result:HoldemResult}){return <section className="th-result-detail"><h3>第 {r.roundNumber} 手 · {r.type==='aborted'?'本手中止':r.type==='fold'?'无需摊牌':'摊牌结果'}</h3><div className="th-report-board">{r.board.map(c=><HoldemCard key={c} card={c} size="report"/>)}</div>{r.pots.map((p,i)=><p key={i}><b>{p.refund?'退回未被跟注部分':i===0?'主池':'边池 '+i} {chips(p.amount)}</b><span>{p.winners.map(w=>r.seats[w].name).join('、')}</span></p>)}{r.seats.map(s=><div className="th-report-player" key={s.id}><b>{s.name} <span className={BigInt(s.delta)>=0n?'positive':'negative'}>{chips(s.delta,true)}</span></b>{s.value&&<span>{s.value.name} · 最佳五张</span>}<div>{s.value?.cards.map(c=><HoldemCard key={c} card={c} size="report"/>)}</div></div>)}</section>}
