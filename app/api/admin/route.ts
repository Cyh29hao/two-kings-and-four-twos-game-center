...
export async function POST(req:Request){return safe(async()=>{const u=await requireUser(req,true),b=await body(req);let description='';
 ...
 if(typeof b.code!=='string')throw new AppError('房间号无效');const r=await getRoom(b.code),g=JSON.parse(r.state) as Game;g.phase='closed';g.deadline=0;g.log.push({text:'管理员已关闭房间，本局不计分',at:Date.now()});
