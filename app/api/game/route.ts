...
export async function POST(req:Request){return safe(async()=>{
 ...
 try{g.rules=landlordRules(b.rules);if(practiceMode(b.mode))fillBots(g);}catch(e){throw new AppError((e as Error).message);}
 ...
 if(['bid','play','pass','double'].includes(b.action)&&g.deadline<=Date.now()){r=await advance(r);throw new AppError('本次操作已超时，已为你自动操作',409);}
