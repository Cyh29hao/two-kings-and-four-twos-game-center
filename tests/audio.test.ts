...
test('holdem calls and street sounds only use public actions',()=>{const g:any={kind:'holdem',seats:[{id:'a',name:'甲'}],phase:'playing'};assert.equal(logCue({text:'甲 全下至 1000',at:1},g)?.speech,'全下');assert.equal(logCue({text:'发出转牌',at:1},g)?.speech,'转牌');assert.equal(logCue({text:'摊牌结算完成',at:1},g)?.effect,'win');assert.equal(logCue({text:'甲 补入 1000 筹码',at:1},g),null);});
