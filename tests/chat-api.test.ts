...
const clients:{data:any;cookie:string}[]=[],suffix=Date.now().toString().slice(-9);for(let i=0;i<3;i++)clients.push(await req('/api/auth',{action:'register',username:'chat'+i+suffix,name:'聊天测试'+i,password:'Chat-local-only-914!'}));
