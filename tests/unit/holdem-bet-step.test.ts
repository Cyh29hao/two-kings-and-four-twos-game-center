...
test('new tables require ante multiples; rejected wagers leave the game untouched',()=>{
 ...
 assert.equal(g.rules.id,'holdem-v2');assert.equal(p.betStep,'10');assert.equal(raiseDetails('81',p,'0').valid,false);
...
test('round minimum up to the next ante multiple after an irregular all-in',()=>{
 const g=game();g.currentBet='85';g.lastRaise='55';g.seats[0].stack='145';
...
test('short calls and explicit all-in retain exact remainder amounts',()=>{
 const g=game(),s=g.seats[g.turn];s.stack='97';moveHoldem(g,s.id,{action:'allin'});assert.equal(g.currentBet,'97');
 const next=g.seats[g.turn];moveHoldem(g,next.id,{action:'call'});assert.equal(next.bet,'97');
 const short=game(),p=short.seats[short.turn];p.stack='17';moveHoldem(short,p.id,{action:'call'});assert.equal(p.bet,'17');assert.equal(p.allIn,true);
});
