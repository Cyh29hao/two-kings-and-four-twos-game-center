...
assert.equal(room.game.rules.id,'ham-v7');assert.equal(room.game.rules.sevenPairsSuitedFourPlus,true);assert.equal(room.game.rules.doubleWildIncludesSelfDraw,true);assert.equal(room.game.rules.closedBonus,false);assert.equal(room.game.rules.forbidWildDiscard,true);assert.equal(room.game.rules.selfDrawUnit,2);assert.equal(room.game.rules.noWildBonus,false);assert.equal(room.game.session.baseChips,'10');
