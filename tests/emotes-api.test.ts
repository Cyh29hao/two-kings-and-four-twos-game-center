...
for(const [path,count] of [['/api/game',3],['/api/mahjong',4],['/api/holdem',10]] as const){
 ...
 const key=crypto.randomUUID(),payload={code,kind:'emote',emoteId:'hello',clientId:key};
 ...
 await req('/api/chat',{...payload,emoteId:'laugh'},clients[0].cookie,409);
 ...
 const attempts=await Promise.all(['laugh','cry','smug'].map(emoteId=>req('/api/chat',{...payload,emoteId,clientId:crypto.randomUUID()},clients[1].cookie)));assert.deepEqual(attempts.map(x=>x.status).sort(),[200,429,429]);
 for(const bad of [{kind:'emote',emoteId:'https://evil/a.png'},{kind:'emote',emoteId:'missing'},{kind:'emote',emoteId:'hello',text:'override'},{kind:'text',text:'hi',emoteId:'hello'},{kind:'unknown'}])await req('/api/chat',{code,clientId:crypto.randomUUID(),...bad},clients[0].cookie,400);
 await req('/api/chat',{code,text:'文字仍然可用 <script>不会执行</script>',clientId:crypto.randomUUID()},clients[0].cookie,200);
 const snapshot=(await req('/api/chat?room='+code,undefined,clients[0].cookie,200)).data;assert.equal(snapshot.messages.length,3);assert.equal(snapshot.messages[0].kind,'emote');assert.equal(snapshot.messages[0].emoteId,'hello');assert.equal(snapshot.messages[0].text,'[表情：打招呼]');assert.equal(snapshot.messages[2].kind,'text');assert(!JSON.stringify(snapshot).includes(clients[0].username));assert(!JSON.stringify(snapshot).includes('clientId'));
...
await req('/api/chat?room='+groups[0].code,undefined,groups[1].clients[0].cookie,403);await req('/api/chat',{code:groups[0].code,kind:'emote',emoteId:'hello',clientId:crypto.randomUUID()},groups[1].clients[0].cookie,403);
...
await req('/api/chat',{code:target.code,kind:'emote',emoteId:'like',clientId:crypto.randomUUID()},target.clients[2].cookie,400);assert((await req('/api/chat?room='+target.code,undefined,target.clients[0].cookie,200)).data.closed);
