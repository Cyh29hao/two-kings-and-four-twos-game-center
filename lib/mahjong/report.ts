...
export type PublicRound = {
  ...
  awards:{tile:number;type:number;hit:boolean}[];players:{name:string;delta:string;balance:string}[];
...
export type Report = {
  ...
  stats:{completed:number;draws:number;aborted:number};players:{name:string;initial:string;delta:string;balance:string}[];
...
export function publicRound(r:RoundResult):PublicRound {
  return {number:r.roundNumber,winner:r.seats[r.winner]?.name??null,source:r.source!==r.winner?r.seats[r.source]?.name??null:null,type:r.winType,ended:r.ended,wildcard:r.wildcard,base:r.baseChips,
    ...
    awards:r.awards.map(a=>({tile:a.tile,type:a.type,hit:a.hit})),players:r.seats.map(s=>({name:s.name,delta:s.delta,balance:s.balance})),
...
export function roundReport(title:string,r:RoundResult):Report {
  return {version:1,scope:'round',title,ruleName:r.rules.name,initialChips:r.initialChips,baseChips:r.baseChips,started:r.started,ended:r.ended,
    stats:{completed:r.winType==='aborted'?0:1,draws:r.winType==='draw'?1:0,aborted:r.winType==='aborted'?1:0},
    players:r.seats.map(s=>({name:s.name,initial:(BigInt(s.balance)-BigInt(s.delta)).toString(),delta:s.delta,balance:s.balance})),rounds:[publicRound(r)],totalRounds:1,page:0,pageSize:20};
}
export function tableReport(title:string,g:ModernGame):Report {
  return {version:1,scope:'table',title,ruleName:g.rules.name,initialChips:g.initialChips,baseChips:g.baseChips,started:g.started,ended:g.ended,stats:{...g.stats},
    players:g.seats.map(s=>({name:s.name,initial:g.initialChips,delta:(BigInt(s.balance)-BigInt(g.initialChips)).toString(),balance:s.balance})),rounds:[],totalRounds:g.stats.completed+g.stats.aborted,page:0,pageSize:20};
}
