...
export function mahjongView(g:MahjongGame,id:string){if(isModern(g))return modern.modernView(g,id);return {...legacy.mahjongView(g,id),schemaVersion:1 as const,
  practice:null,wildcard:-1,result:null,session:null,entries:[],canChoose:false,awardReveal:null,seats:legacy.mahjongView(g,id).seats.map(s=>({...s,bot:false,balance:null}))};}
