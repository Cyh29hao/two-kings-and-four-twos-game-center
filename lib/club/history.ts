...
export function roundSummary(row:{id:string;room_code:string;created:number;result:string},userId:string):HistoryItem{
 const r=JSON.parse(row.result),game=(r.kind??'landlord') as HistoryGame,me=r.seats.find((s:any)=>s.id===userId);if(!me)throw Error('Not a participant');
 const label=game==='landlord'?(r.winner===r.landlord?'地主获胜':'农民获胜'):game==='holdem'?({showdown:'摊牌结算',fold:'无需摊牌',aborted:'本手中止'}[r.type as string]??'已结算'):r.winType==='draw'?'流局':r.winType==='aborted'?'本局中止':`${r.seats[r.winner]?.name??'牌友'}${r.winType==='self'?'自摸':'胡牌'}`;
 return {id:row.id,code:row.room_code,created:row.created,game,title:r.roundNumber?`第 ${r.roundNumber} ${game==='holdem'?'手':'局'}`:'单局记录',label,delta:String(me.delta??0),unit:game==='holdem'||game==='mahjong'&&r.schemaVersion===2?'筹码':'积分',practice:!!r.practice||r.seats.some((s:any)=>s.bot),modern:game==='mahjong'&&r.schemaVersion===2};
}
