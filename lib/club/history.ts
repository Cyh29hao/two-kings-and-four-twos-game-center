...
export function roundSummary(row:{id:string;room_code:string;created:number;result:string},userId:string):HistoryItem{
 ...
 const label=v3?`${r.seats[r.winner]?.name??'牌友'} 获得扩展赛制胜利`:game==='landlord'?(r.winner===r.landlord?'地主获胜':'农民获胜'):game==='holdem'?({showdown:'摊牌结算',fold:'无需摊牌',aborted:'本手中止'}[r.type as string]??'已结算'):r.winType==='draw'?'流局':r.winType==='aborted'?'本局中止':`${r.seats[r.winner]?.name??'牌友'}${r.winType==='self'?'自摸':'胡牌'}`;
