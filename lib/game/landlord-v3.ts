...
export function viewV3(g:V3Game,id:string){const own=g.seats.findIndex(seat=>seat.id===id);return {...g,visualEvents:publicVisuals(g),tableActions:actions(g),shops:g.shops.map((shop,index)=>index===own?{offers:shop.offers.map(offer=>({...offer}))}:{offers:[]}),equipment:g.equipment.map((items,index)=>items.map(item=>index===own?{...item}:{id:item.id,level:item.level})),seats:g.seats.map(seat=>({...seat,count:seat.hand.length,hand:seat.id===id||g.phase==='finished'?seat.hand:[]}))};}
