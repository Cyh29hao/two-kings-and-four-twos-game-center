...
import {holdemView,moveHoldem,type HoldemGame,type HoldemView,type HoldemMove} from './engine.ts';
...
export function holdemDecision(v:HoldemView,id:string):HoldemMove{
 ...
 if(o.canRaise&&aggression){let target=BigInt(Math.floor(Number(me.bet)+call+Math.max(big,pot*(equity>.78?.85:.55))));if(target<BigInt(o.minRaise))target=BigInt(o.minRaise);if(target>BigInt(o.maxRaise))target=BigInt(o.maxRaise);return{action:'raise',amount:target.toString()};}
