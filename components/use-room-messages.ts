...
import type {ChatMessage,ChatSnapshot} from '@/lib/chat-bubbles';
...
export function useRoomMessages(code:string,onSnapshot?:(snapshot:ChatSnapshot)=>void){
 ...
 const controls=useRef<{code:string;refresh:()=>Promise<void>;signal:AbortSignal}|null>(null);
 useEffect(()=>{
  ...
  async function refresh():Promise<void>{
   ...
   const run=async()=>{
    ...
    try{
     ...
     setState(prev=>({code,messages:mergeMessages(prev.code===code?prev.messages:[],data.messages,full),initialized:true,closed:data.closed,readyAt:Date.now()+Math.max(0,data.emoteReadyAt-data.serverNow),error:''}));
   ...
   if(live&&!blocked&&!document.hidden&&!timer)timer=setTimeout(()=>{timer=undefined;void refresh()},Math.min(1500*2**failed,15000));
  }
  controls.current={code,refresh,signal:lifetime.signal};
 ...
 return {...(state.code===code?state:{code,messages:[],initialized:false,closed:false,readyAt:0,error:''}),refresh:async()=>{if(controls.current?.code===code)await controls.current.refresh();},signal:()=>controls.current?.code===code?controls.current.signal:AbortSignal.abort()};
}
