...
import {newDraft,reconcileDraft,selectAction,paymentDetails,quickPayment,raiseUnit,canSubmitAction,type BettingAction,type PreselectionContext} from '@/lib/holdem/preselection';
...
export function HoldemControls({game:g,userId,roomCode,connected,reset,seconds,busy,onAction,onTimeoutChoice,children}:Props){
 ...
 const unit=preview?raiseUnit(preview,g.currentBet):'0';
 ...
 function addPayment(multiple:string){if(preview&&me)fillPayment(quickPayment(draft.raise,preview,me.bet,g.currentBet,multiple));}
 ...
 return <section className="th-controls table-dock" aria-label="我的德州操作">
  ...
  {betting?<div className="table-action-area" aria-label={context.acting?'当前行动':'提前准备'}>
   ...
   {wagerOpen&&<div className="table-choice-panel th-wager-panel" id="holdem-wager-panel" aria-label="下注金额">
    ...
    <div className="th-quick-payments" role="group" aria-label="快速填入金额"><Button variant="outline" disabled={!eligible||busy} onClick={()=>fillPayment(preview?.call??'0')}>跟注额 {chips(preview?.call??'0')}</Button>{['1','2','5'].map(amount=><Button key={amount} variant="outline" disabled={!eligible||busy} title={`增加 ${chips((BigInt(unit)*BigInt(amount)).toString())}，按本桌下注单位取整`} onClick={()=>addPayment(amount)}>+{amount}倍</Button>)}</div>
    <small className="th-wager-basis">1 倍 = {chips(unit)}（当前最低加注幅度）</small></div><div className="th-wager-summary"><span role="status" aria-live="polite">将付出 <strong>{draft.selected?.action==='allin'?chips(me.stack):details?.extra==null?'—':chips(details.extra)}</strong> · 剩余 {draft.selected?.action==='allin'?'0':details?.remaining===null?'—':chips(details?.remaining??me.stack)}</span><small>最低投入 {chips((BigInt(preview?.minRaise??'0')-BigInt(me.bet)).toString())}{preview?.betStep!=='1'&&<> · 本轮加注至须为 {chips(preview?.betStep??'1')} 的倍数</>}</small></div>
