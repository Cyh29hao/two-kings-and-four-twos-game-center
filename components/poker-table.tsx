...
export function SeatAction({action,active,seconds,bidding,doubling=false,equipment=false,name}:{action?:TableAction|null;active:boolean;seconds:number;bidding:boolean;doubling?:boolean;equipment?:boolean;name:string}){
...
export function HandStrip({hand,selected,onChange,round}:{hand:number[];selected:number[];onChange:(cards:number[])=>void;round:string}){
 ...
 function paint(ids:number[]){const g=gesture.current;if(!g)return;const next=brushSelection(selection.current,ids,g.select,g.visited);selection.current=next;onChange(next);}
