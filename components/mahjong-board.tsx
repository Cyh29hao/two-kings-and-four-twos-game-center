...
export function MahjongSurface({g,seat,seconds,children,botControls,bubbles={}}:{g:MahjongView;seat:number;seconds:number;children?:ReactNode;botControls?:BotSeatControls;bubbles?:Record<string,ChatBubble>}){
 ...
 <div className="mj-center">{g.phase==='finished'?<span className="mj-finish-seal">本局<br/>结束</span>:g.phase==='playing'?<div className="mj-wall-count"><span>余牌</span><strong>{g.remaining}</strong></div>:children}</div>
...
export function ExpandMahjong({g,seat,seconds,bubbles={}}:{g:MahjongView;seat:number;seconds:number;bubbles?:Record<string,ChatBubble>}){return <Dialog><DialogTrigger asChild><Button variant="ghost" size="icon" className="mj-expand table-tool" aria-label="完整牌桌" title="完整牌桌"><Maximize2 size={18}/></Button></DialogTrigger><DialogContent className="mj-expanded-modal"><DialogHeader><DialogTitle>完整牌桌</DialogTitle><DialogDescription>左右滑动查看四家的完整弃牌与吃碰杠。自己的手牌在下方。</DialogDescription></DialogHeader><div className="mj-expanded-scroll"><div className="mj-board mj-expanded-board"><MahjongSurface bubbles={bubbles} g={g} seat={seat} seconds={seconds}/><div className="mj-self"><div className="mj-self-top"><div><PlayerChat bubble={bubbles[g.seats[seat].id]} align="start"><span className="avatar">{g.seats[seat].name.slice(0,1)}</span></PlayerChat><strong>{g.seats[seat].name}</strong><span>你</span></div></div><div className="mj-hand" aria-label="你的手牌">{g.seats[seat].hand.map(t=><MahjongTile key={t} tile={t} wildcard={g.wildcard}/>)}</div></div></div></div></DialogContent></Dialog>;}
